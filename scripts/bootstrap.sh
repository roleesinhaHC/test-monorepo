...
rm "$PWD/cloud/workspaces.gen.tf"

echo "Generating cloud/workspaces.gen.tf..."

## ~Generate cloud workspace workspaces.gen.tf~
for ((i=1;i<=DIRCOUNT;i++)) do
  export INDEX=$i PROJECT=$PROJECT
  TF_RESOUCE=$(envsubst '$INDEX:$PROJECT' < "$PWD/scripts/workspaces.tf.template" )
  echo -e "$TF_RESOUCE" >> "$PWD/cloud/workspaces.gen.tf";
done
